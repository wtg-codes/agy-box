...
# Install kubectl
KUBECTL_VERSION=$(curl -L -s --http1.1 --connect-timeout 5 --retry 5 --retry-delay 2 https://dl.k8s.io/release/stable.txt)
curl -LO --http1.1 --connect-timeout 5 --retry 5 --retry-delay 2 "https://dl.k8s.io/release/${KUBECTL_VERSION}/bin/linux/amd64/kubectl"
curl -LO --http1.1 --connect-timeout 5 --retry 5 --retry-delay 2 "https://dl.k8s.io/release/${KUBECTL_VERSION}/bin/linux/amd64/kubectl.sha256"
echo "$(cat kubectl.sha256)  kubectl" | sha256sum -c -
install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl
rm kubectl kubectl.sha256

# Install k9s
K9S_VERSION=$(curl -sS --http1.1 --connect-timeout 5 --retry 5 --retry-delay 2 https://api.github.com/repos/derailed/k9s/releases/latest | jq -r .tag_name)
...
# Install helm
curl -fsSL --http1.1 --connect-timeout 5 --retry 5 --retry-delay 2 https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 -o get_helm.sh
